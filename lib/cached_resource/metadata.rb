    ...
    class Metadata
      ...
      def self.fetch(klass)
        object = CACHE_STORE.fetch(klass.to_s) do
          ...
          object && CachedResource::log("WRITE METADATA #{klass}")
          object
        end

        object && CachedResource::log("READ METADATA #{klass}")
        object
      end

      def save
        object = CACHE_STORE.write(class_name, self)
        CachedResource::log("METADATA #{self.to_json}")
        object && CachedResource::log("WRITE METADATA #{class_name}")
      end
